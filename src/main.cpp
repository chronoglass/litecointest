...
#if defined(NDEBUG)
# error "Bipscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x85ca917242794727768581eaa4d0ac3e06e7109f449ff3925479edb777b56208");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bipscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bipscoin Signed Message:\n";
...
{
    // Bipscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Bipscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Bipscoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Bipscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bipscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bipscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6036800a616c991eb6073c9c34830d94e8263005402f6e2020e6f0f91c5d6f0c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "France Terror Arrest: How the Alleged Paris Plot Was Thwarted";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Bipscoin: increase each by adding 2 to bitcoin's value.
